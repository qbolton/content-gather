<?php
class ExtractJob extends CGJob {

  public function start($options = NULL) {
    parent::start($options); print_r($options);
    $this->logger->info("<<<<<<<<<<<<");
    ...
    foreach ($feeds as $feed) {
      ...
      if ( (!is_null($raw_json)) && (is_object($raw_json)) && ($raw_json->success) ) {
        ...
        foreach($feed_items as $raw_feed_item) {
          ...
          // prepare to extract the url's content with embedly's API
          $post = NULL;

          $this->logger->debug("Using Embedly's API to extract the URL content");

          // Call to embedly wrapper method
          $content = Embedly::extract($raw_feed_item->link);

          // create object to hold info needed to pass to post object
          $content->site_id = $site_id;
          $content->fid = $feed_id;
          // add in publication date
          if (isset($raw_feed_item->pubDate)) {
            $content->pubDate = $raw_feed_item->pubDate;
          }

          //print_r($content); exit;

          // break the feed item up into it's meta detail fields
          $post = new ExtractParser($content); $post->parse(); // print_r($post);
