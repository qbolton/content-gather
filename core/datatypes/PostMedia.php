...
class PostMedia extends CGObject {
  ...
  static public function getById($id,$returnArray=TRUE) {
    ...
    $results = NULL;
    $table = new CGPostMediaTable ($conn);
    $result = $table->select()->where("pmid=".$id)->run(TRUE);
    // if rows
    if (!$returnArray) {
      foreach ($result as $row) {
        $results[] = new PostMedia($conn,$row);
      }
    }
    else {
  ...
  static public function getByName($name,$returnArray=TRUE) {
    ...
    $results = NULL;
    $table = new CgPostMediaTable ($conn);
    $result = $table->select()->where("media_name='".strtolower(trim($name))."'")->run(TRUE);
    // if rows
    if (!$returnArray) {
      foreach ($result as $row) {
        $results[] = new PostMedia($conn,$row);
